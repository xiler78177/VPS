...
3. **编辑 `cdn-preferip.conf`**，填 A 步记下的值：
   ...
   其中 `-tlr 0` 要求零丢包，`-sl 1` 要求下载速度至少 1 MB/s，`latency` 表示在满足这些门槛后优先选平均延迟最低的 IP。
...
4. **编辑 `nodes.txt`（多节点推荐）**，给每个节点标注实际落地区域对应的 CF colo/IATA 码：
   ...
   格式为 `备注|CF地区码|vless链接`。地区码用于 B 阶段按组执行 `CloudflareSpeedTest -httping -cfcolo`；C 阶段回写时，每个节点只使用自己地区组的优选 IP。旧格式 `备注|vless链接` 仍兼容，但会走 `DEFAULT_CF_COLO` 或全局优选。
...
6. **把客户端订阅这条专用订阅的下载链接**。以后客户端刷新订阅，就会自动拿到按节点地区匹配后的最新优选 IP。
...
```
[国内机] 20:30 cron
   │  B: 按 nodes.txt 的 HKG/ICN/NRT... 分组优选 CF 边缘 IP
   ▼
   │  C: PATCH 专用订阅 → 每个节点 server=对应地区优选IP, host/sni=cdn域名
   ▼
...
| 单次优选结果波动大 | 设置 `CFST_ROUNDS=3` 多跑几轮；带宽已有门槛时用 `CFST_PICK_MODE=latency` 降低延迟，或用 `balanced` 优先多轮命中 |
| 需要 IPv6 优选池 | XIU2/CloudflareSpeedTest 自带 `ipv6.txt`，在 conf 里设置 `CFST_IP_FILE="/opt/cfst/ipv6.txt"`；脚本会自动给 IPv6 server 加 `[]` |
