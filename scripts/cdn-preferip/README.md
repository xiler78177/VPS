...
| **C** 回写 sub-store | **国内机** | 把优选 IP PATCH 进一条专用订阅（默认写 server；可选同步入口域名 DNS） | 定时（紧跟 B） |
...
3. **编辑 `cdn-preferip.conf`**，填 A 步记下的值：
   ...
   CFST_TOP_N="1"                 # 每个地区取前 N 个最优 IP；>1 时可给同地区多节点做 IP 池化
   ...
   如果你不想依赖客户端及时刷新订阅，可以把每个节点再加一个入口域名：
   ...
   这样脚本会把订阅里的 `server` 写成 `prefer-hkg.example.com`，同时把这条 DNS 记录同步到当前优选 IP。
   客户端即使短时间没刷新订阅，也会在 DNS 过期后跟随新 IP。
...
5. **先手动跑一次串联脚本**，确认全链路通：
   ...
   - 之后每次：C `PATCH` 这条订阅，默认只替换 server 为最新优选 IP；若启用 DNS 入口模式，则同步更新入口域名的 A/AAAA（host/sni 永远是真实域名）。
   - 成功后到 sub-store 面板能看到名为 `cdn-preferip` 的订阅，里面是 CDN 节点。
   - 脚本会维护本地 `preferip.state.tsv`、`preferip.history.csv`、`bad-ip.txt` 和 `backup/`：
     用于稳态切换、历史追踪、临时黑名单和 PATCH 前订阅备份。

6. **把客户端订阅这条专用订阅的下载链接**。以后客户端刷新订阅，就会自动拿到按节点地区匹配后的最新优选 IP；若你启用了 DNS 入口模式，客户端短时间不刷新也会在 DNS TTL 后跟上新 IP。
