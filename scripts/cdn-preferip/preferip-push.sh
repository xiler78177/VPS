...
#
# 多节点模式(推荐)：读 nodes.txt(每行「区域备注|vless链接」)，把每个节点的 server 换成
#   当天优选 IP、备注换成你写的区域名，host/sni/uuid/path 全保留，一次性 PATCH 到专用订阅。
#   —— 优选 IP 是「你→CF边缘」的最优解，对所有节点通用，故一次优选服务全部节点。
...
best="${ips[0]}"   # 多节点共用 top1 优选 IP
log "优选 IP=${best}（共 ${#ips[@]} 个候选）"

# 2) 拼装节点内容
content=""
n=0
if [[ -f "$NODES_FILE" ]]; then
    # ── 多节点模式：nodes.txt 每行「区域备注|vless链接」 ──
    while IFS= read -r raw || [[ -n "$raw" ]]; do
        [[ -z "${raw//[[:space:]]/}" ]] && continue                  # 空行
        trimmed="${raw#"${raw%%[![:space:]]*}"}"                     # 去行首空白
        [[ "$trimmed" == \#* ]] && continue                          # 行首#注释
        note="${raw%%|*}"
        link="${raw#*|}"
        note="${note#"${note%%[![:space:]]*}"}"; note="${note%"${note##*[![:space:]]}"}"
        link="${link#"${link%%[![:space:]]*}"}"; link="${link%"${link##*[![:space:]]}"}"
        if [[ "$link" != vless://* || -z "$note" ]]; then
            log "跳过无效行（需「备注|vless链接」）: $raw"; continue
        fi
        new="$(rewrite_vless "$link" "$best" "$note")" || { log "解析失败，跳过: $note"; continue; }
        content+="${new}"$'\n'
        n=$((n+1))
    done < "$NODES_FILE"
    log "多节点模式：从 nodes.txt 生成 ${n} 个节点（server 全部=${best}）"
    [[ $n -gt 0 ]] || die "nodes.txt 里没有有效节点（每行格式：区域备注|vless链接）"
else
    # ── 单节点模式（兼容旧 conf）──
...
log "完成：${SUBSTORE_SUB_NAME} 已更新 ${n} 个 CDN 节点（server=${best}）。客户端刷新订阅即生效。"
