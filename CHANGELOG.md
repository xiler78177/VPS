...
### Fixed
- **[P2] OpenWrt WireGuard 卸载残留 rc.local 持久化片段**：`wg_uninstall`（OpenWrt）清理了 UCI、看门狗、配置文件，却未清 `/etc/rc.local` 中本脚本托管的 WireGuard 片段（allow-port UDP 放行、bypass 动态解析块等），卸载后重启这些片段仍会执行、报错刷屏且可能重放已失效的防火墙规则。回滚路径本就调用 `_wg_rc_local_cleanup_managed_entries all`，卸载路径却漏了。修复：卸载在提交 UCI 清理后补一次 `_wg_rc_local_cleanup_managed_entries all`，失败仅告警不中止。
- **[P2] WireGuard 网关 LAN 子网重叠无检测**：添加网关设备时，其 LAN 网段若与 VPN 子网 / 服务端 LAN / 其他网关 LAN 重叠，会造成 VPN 内路由目标不唯一、跨站点访问静默失败，原代码仅有一句静态文字提示、不做实际计算。修复：新增 `01-utils.sh` 的 IPv4 CIDR 重叠判定 `cidr_overlap`/`cidr_overlaps_list`（IPv6/非法输入安全跳过），OpenWrt(`11d`) 与 Debian(`12d`) 添加网关流程在写库前实检重叠并告警确认；新增 7 条 `core_ops_mock_test.sh` 断言。
- **[P2] 远程网关一键部署命令含默认路由时无自断告警**：`_wg_show_openwrt_deploy` 生成的 OpenWrt 部署命令在 `route_allowed_ips=1` 下，若网关 AllowedIPs 含默认路由（`0.0.0.0/0`/`::/0`），粘贴执行会把目标路由器默认路由切入隧道，可能瞬间切断正通过公网 SSH 操作该路由器的管理员会话。修复：展示部署命令前检测默认路由并醒目告警，建议改走 LAN 侧/本地控制台或先备好带外恢复通道。
- **[P2] Debian WireGuard 修改出口网卡不校验存在性**：`wg_deb_modify_server` 直接接受输入的出口网卡名并写库，拼写错误/不存在的网卡会导致 NAT masquerade 出口静默失效、VPN 客户端无法上网。修复：校验网卡名格式，并对 `/sys/class/net` 中不存在的网卡告警确认（保留非常规命名的手动确认路径）。
...
- **[P1] OpenWrt 联动更新客户端路由会漏掉 managed 类 peer**：`_wg_update_peer_routes`（OpenWrt）在取 `route_mode` 之前就无条件用 `[[ "$_cur" == "$server_subnet" ]]` 短路，把「当前 AllowedIPs 恰等于 VPN 子网」的 peer 全部跳过——但服务端 `server_lan` 为空且无网关时新增的 clash/标准（`managed`）peer 正好命中此条件，导致后续添加网关 peer 时它拿不到新网关的 LAN 网段路由，只能删重建。Debian 版 `_wg_deb_update_peer_routes` 逻辑正确。修复：OpenWrt 版对齐 Debian 版——先按 `route_mode` 分类跳过 `custom|full|vpn`，`server_subnet` 短路限定在 `-z "$_route_mode"`（旧数据兼容），并补 `::/0` 判断。
- **[P1] 禁用密码登录的防误锁检查不解析 AllowUsers/DenyUsers 致可能锁外**：`disable_password_auth` 仅用 `_ssh_authorized_keys_available` 检查是否存在含公钥的 `authorized_keys`，不校验这些用户是否真被 sshd 允许登录。若管理员配了 `AllowUsers alice` 但只有 `bob` 有密钥，检测通过、禁用密码并重启后无人能登录。修复：新增 `_ssh_login_policy_allows`，用 `sshd -T -C user=<user>` 逐用户解析有效 `AllowUsers/AllowGroups/DenyUsers/PermitRootLogin`（支持 Match 块与通配符），`_ssh_authorized_keys_available` 改为只认「既有密钥又被允许登录」的用户；无 sshd/解析失败时保持原放行行为，不引入新的误判。
- **[P1] Debian WireGuard 安装不校验密钥生成成功**：`12c` 服务端安装 `server_privkey=$(wg genkey)` 后未校验非空即写入 DB 与 `wg0.conf`（OpenWrt `11c` 已校验），`wg genkey` 失败时空密钥被写入、直到 `wg-quick` 启动才报错且已污染 DB。修复：与 OpenWrt 版对齐，生成后校验非空，失败即中止并回滚已放行的 UDP 端口。
- **[P2] Cloudflare DNS 删除只删首条匹配记录**：`_cf_dns_delete` 取 `.result[0].id` 只删一条，同名存在多条历史残留记录（如多条同名 CNAME）时只清一条，剩余记录与新建记录冲突导致建站/回源失败。修复：遍历 `.result[].id` 删除全部匹配记录，任一删除失败即返回错误。
- **[P2] 家宽暴露自动探测的公网 IP 未校验即写入 CF DNS**：`web_home_expose` 自动探测分支 `get_public_ipv4` 后仅判空、未做 `validate_ip`（手动输入分支已校验），畸形探测结果会污染 A 记录。修复：无论来源都过 `validate_ip` 并排除 IPv6，失败回退手动输入。
- **[P2] WARP 出口 IP 判断白名单过窄致 IPv6-only+WARP 落地机漏判**：`reality_ipv4_is_likely_warp_egress` 原仅匹配 `104.28/104.29` 两段，且兜底 `reality_has_warp_interface` 只按接口名（`warp|wgcf|cloudflare`）匹配——wgcf 隧道落在 `wg0` 时漏判，WARP 出口 IP 被误写入 CF 灰云 A 记录致节点不可达。修复：扩充 egress 段至 `104.16-31`，新增 `reality_has_warp_wg_endpoint` 按 `wg show all endpoints` 的 Cloudflare WARP 服务端 IP 段（`162.159.192/193/195`、`188.114.96-99`、`2606:4700:d0/d1`）判定，不依赖接口名。
