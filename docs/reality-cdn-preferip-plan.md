...
> 状态：**设计阶段，未实现**。本文档只描述方案与落地计划，不含已落地代码。
> 目标读者：本仓库维护者（决定是否、以及怎么把它做进 `modules/`）。
...
```
   ...
   落地 VPS（海外）─────┤
                        └─ [新增] CDN 链路（橙云 + 优选 IP）
                              ...
                              回源落点：nginx :443（cdn.域名 真实证书，DNS-01 签发）
                                        └ location /<隐秘路径>  →反代→ 127.0.0.1:<内部端口>
                                                                         (sing-box VLESS-WS 入站)
                                        └ location /            → 伪装站（正常网页，避免裸 IP 探测穿帮）
...
复用现有 09 web 栈（已确认决策）。nginx 在 443 上：
- 用真实域名证书做 TLS（CF 橙云 ↔ 源站这一段是「Full (strict)」加密，需要有效证书）。
- 把隐秘 WS 路径反代到 sing-box 内部端口，其余路径回一个正常网页做伪装。
...
## 4. 分块交付计划（本次仅文档，后续按块实现）

三块**可独立交付、独立验证**。A 是主体且全在本仓库内。

### A. 源站向导（落地机，本仓库主体）

在 `15-singbox-reality.sh`（或新模块 `15b-singbox-cdn.sh`）新增「为 Reality 节点加挂 CDN 链路」向导：
...
**部署实测**（SanJose-ARM-1 / `165.1.67.252`，Debian 12 aarch64）：

- sub-store 跑在 Docker（`xream/sub-store:latest`），后端 3000 → 宿主 **`127.0.0.1:3001`**、
  前端 3001 → 宿主 `127.0.0.1:3002`，**仅回环监听**，由 nginx `sub.niubao.cf.conf` 反代
  （`location /` → `127.0.0.1:3002`）。
- **API secret 前缀**：`SUB_STORE_BACKEND_PREFIX=/9Gg78hWFEguXZBT3oHPY`（即 URL 里的密钥段）。
  真实 API base = `http://127.0.0.1:3001/9Gg78hWFEguXZBT3oHPY/api`（本机直连）或
  `https://sub.niubao.cf/9Gg78hWFEguXZBT3oHPY/api`（公网）。
...
- **C 脚本的落点**：跑在 SanJose 这台机上最省事——直连 `127.0.0.1:3001` + secret 前缀，
  无需公网、无需穿透。但**优选 IP 必须来自国内机（B）**，所以 B→C 之间要把 IP 列表
  从国内机送到 SanJose（或 C 直接在国内机跑、走公网 `https://sub.niubao.cf/<secret>/api` PATCH）。
  二选一在实现 C 前定（见下）。
...
- **A 落地前需确认**：CDN state 的字段设计、`reality_render_singbox_config` 的改造方式（合并渲染）
  是否如 §3、§4.A 所述。
- **C 落地前需定**：B→C 的 IP 传输路径——C 跑在 SanJose 直连 `127.0.0.1:3001`（需把国内机 B 的
  优选结果送过来），还是 C 直接在国内机跑、走公网 `https://sub.niubao.cf/<secret>/api` PATCH。
