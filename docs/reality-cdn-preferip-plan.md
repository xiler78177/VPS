...
- **默认优选不改 DNS**。早期实现是让客户端节点的「服务器地址」字段直接写优选出的 CF 边缘 IP，
  而 SNI / Host 头继续填真实域名，自动更新的是**订阅里节点的 server 字段**。
  现在额外支持**入口域名模式**：订阅里写固定 entry 域名，脚本同步把该域名的 A/AAAA 记录指向优选 IP，
  这样客户端即使没及时拉订阅，也会跟随 DNS 变化。
...
6. 产出客户端链接/JSON（`vless://...?type=ws&path=...&host=cdn.域名&sni=cdn.域名`，
   server 字段默认先填 `cdn.域名`，优选后由 C 替换为优选 IP；若启用入口域名模式，则保持入口域名并同步 DNS）。
...
B 产出最优 IP 后，脚本对这条订阅 `PATCH /api/sub/<name>`，默认把节点 server 字段替换为优选 IP；
若启用入口域名模式，则保留 server 为入口域名，并把该域名的 A/AAAA 记录同步到优选 IP
（host/sni 始终保持真实域名 `cdn.域名`）。客户端订阅该专用订阅的下载链接，刷新即得新 IP。
